# Generated by Powerlevel10k configuration wizard on 2025-08-25 at 16:47 PDT.
...
() {
  ...
  function prompt_mise() {
    local plugins=("${(@f)$(mise ls --current --offline 2>/dev/null | \
      awk '!/\(symlink\)/ && $3!="~/.tool-versions" && \
           $3!="~/.config/mise/config.toml" && $3!="(missing)" {print $1, $2}')}")
    local plugin
    for plugin in ${(k)plugins}; do
      local parts=("${(@s/ /)plugin}")
      # php-brew fix
      local raw_tool=${parts[1]//-brew/}
      ...
      local fg="${(P)fg_var}"
      local bg="${(P)bg_var}"
      p10k segment -r -f $fg -b $bg -t "$icon $version"
    done
  }
